    ...
    public void auditEvent(String key, String description, String user, EventsUsageData usageEvent, StatusUsageEvent status,
        ...
        usageDataService.call(new RemoteCallback<Void>() {
            ...
            public void callback(Void nothing) {
                ...
                new UsageEventSummaryBuilder().key(key).description(description).from(user)
                        .component(usageEvent.getComponent()).action(usageEvent.getAction()).module(usageEvent.getModule())
                        .status(status.toString()).level(level.toString()).build());
