...
public class UDCVfsServiceImpl extends UDCVfsManager implements UDCVfsService {
    ...
    public GfsSummary getInfoGfs() {
        ...
        try {
            ...
            log.error("Error getting information about Gfs ");
            gfsSummary = null;
        }
        return gfsSummary;
    }
    ...
    private void addToInbox(String boxName, String itemPath, String note, String userFrom, String toUser) {
        ...
        Queue<UsageEventSummary> entries = removeAnyExisting(note, readEntries(toUser, boxName));
        checkSizeQueue(entries);
        entries.add(new UsageEventSummaryBuilder().key(boxName).description(note).component(boxName)
    ...
    private Queue<UsageEventSummary> removeAnyExisting(String note, Queue<UsageEventSummary> inboxEntries) {
        Iterator<UsageEventSummary> it = inboxEntries.iterator();
        while (it.hasNext()) {
            UsageEventSummary e = it.next();
            if (e.getDescription().equals(note)) {
                it.remove();
                return inboxEntries;
            }
        }
