...
/**
 * This service stored in a QueueSession all Usage Data. recent_edited,
 * recent_viewed and icoming are stored in vfs
 */
@Service
@ApplicationScoped
public class UDCSessionServiceImpl extends UDCSessionManager implements UDCSessionService {
    ...
    public Queue<UsageEventSummary> readUsageDataCollector() {
        return (super.getSession().getAttribute(KEY_EVENTS) != null) ? (Queue<UsageEventSummary>) super.getSession()
                .getAttribute(KEY_EVENTS) : null;
    }
