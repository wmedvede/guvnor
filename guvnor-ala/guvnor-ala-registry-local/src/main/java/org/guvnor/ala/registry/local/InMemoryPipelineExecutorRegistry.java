...
public class InMemoryPipelineExecutorRegistry
    ...
    @Override
    public void register(PipelineExecutorTrace record) {
        PortablePreconditions.checkNotNull("record", record);
        recordsMap.put(record.getTaskId(), record);
    }

    /**
     * @return the PipelineExecutorTrace with the information of the pipeline execution that generated the given
     * runtime. If the runtime wasn't generated by a pipeline, or the PipelineExecutorTrace was not found returns null.
     */
    @Override
    public PipelineExecutorTrace getExecutorTrace(RuntimeId runtimeId) {
        return recordsMap.values().stream()
                .filter( record -> runtimeId.equals(record.getTask().getTaskDef().getRuntimeId()))
                .findFirst()
                .orElse(null);
    }
    ...
    @Override
    public Collection<PipelineExecutorTrace> getExecutorTraces(String pipelineId) {
