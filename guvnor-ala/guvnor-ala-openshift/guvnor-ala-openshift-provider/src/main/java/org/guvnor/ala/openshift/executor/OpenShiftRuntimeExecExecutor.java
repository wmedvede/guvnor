...
public class OpenShiftRuntimeExecExecutor<T extends OpenShiftRuntimeConfig> implements
        RuntimeBuilder<T, OpenShiftRuntime>,
        RuntimeDestroyer,
    ...
    private Optional<OpenShiftRuntime> create( final OpenShiftRuntimeConfig runtimeConfig ) throws ProvisioningException {
        ...
        final String id = new OpenShiftRuntimeId(
                openshiftClient.getNamespace(),
                runtimeConfig.getServiceName(),
                runtimeConfig.getApplicationName() ).toString();
        LOG.info( "Created runtime: " + id );
        
        OpenShiftRuntimeEndpoint endpoint = openshiftClient.getRuntimeEndpoint(id);
        
        return Optional.of( new OpenShiftRuntime( id, buildRuntimeName(runtimeConfig, id), runtimeConfig, openshiftProvider,
                endpoint, new OpenShiftRuntimeInfo(runtimeConfig), runtimeState ) );
    }
